...
Docs are [Specular](https://www.earlyeffect.rocks/specular/) DocSpecs: examples assert in CI, diagrams are mermoid renders, and the live widgets are a real `Conduit` (ascent is a **docs-only** dependency).

**[→ conduit on earlyeffect.rocks](https://www.earlyeffect.rocks/conduit)** (`sbt docsPreview` locally)

| Topic | What's in it |
| --- | --- |
| Overview | Dispatch loop, install |
| Getting started | Model → action → handler → live counter |
| Mental model | `run` vs `run(false)`, FastEq gates, nested follow-ups |
| Lenses and optics | `Optics` macro, composition, laws |
| Handlers | `update` / `focus` / `>>` vs `++`, follow-ups |
| Listeners | Slice subscriptions, live log |
| FastEq | Skip notify; live skip vs fire |
| Collection lenses | `at` / `key`, live todos |
| Iso | `imap` / `xmap`, store Int / show String |

The library cross-builds for **JVM, Scala.js, and Scala Native**. Ascent is not on the published classpath.
...
```scala
libraryDependencies += "rocks.earlyeffect" %% "conduit" % "x.y.z"
```

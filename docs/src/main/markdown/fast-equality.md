...
```scala marklit:silent,id=fast-base
import conduit.*
import zio.*
...
```scala marklit:zio-app,extends=fast-base
for
...
```scala marklit:extends=dirty-eq
...
```scala marklit:silent,id=fasteq-get
import conduit.*
...
```scala marklit:silent,id=builtin
import conduit.*
